...
def fetch_campaign_metadata(campaign_id_list: list[str]) -> pd.DataFrame:
    ...
    if not campaign_id_list:
        fetch_section_succeeded["1.1.2. Validate input for TikTok Ads campaign metadata fetching"] = False
        fetch_section_failed.append("1.1.2. Validate input for TikTok Ads campaign metadata fetching")
...
def fetch_ad_metadata(ad_id_list: list[str]) -> pd.DataFrame:
    ...
    # 1.2.10. Enforce schema for Python DataFrame
    try:
...
def fetch_ad_creative() -> pd.DataFrame:
    ...
    # 1.3.1. Start timing the TikTok Ads ad metadata fetching process
    start_time = time.time()
    fetch_section_succeeded = {}
    fetch_section_failed = []
    ...
    try:
        ...
        print(f"✅ [FETCH] Successfully initialized Google Secret Manager client for Google Cloud project {PROJECT}.")
        logging.info(f"✅ [FETCH] Successfully initialized Google Secret Manager client for Google Cloud project {PROJECT}.")
        fetch_section_succeeded["1.3.2 Initialize Google Secret Manager client"] = True
    except Exception as e:
        fetch_section_succeeded["1.3.2 Initialize Google Secret Manager client"] = False
        fetch_section_failed.append("1.3.2 Initialize Google Secret Manager client")
        print(f"❌ [FETCH] Failed to initialize Google Secret Manager client for Google Cloud Platform project {PROJECT} due to {e}.")
        logging.error(f"❌ [FETCH] Failed to initialize Google Secret Manager client for Google Cloud Platform project {PROJECT} due to {e}.")
        raise RuntimeError(f"❌ [FETCH] Failed to initialize Google Secret Manager client for Google Cloud Platform project {PROJECT} due to {e}.") from e
    ...
    try: 
        ...
        print(f"✅ [FETCH] Successfully retrieved TikTok access token for account {ACCOUNT} from Google Secret Manager.")
        logging.info(f"✅ [FETCH] Successfully retrieved TikTok access token for account {ACCOUNT} from Google Secret Manager.")
        fetch_section_succeeded["1.3.3. Get TikTok Ads access token from Google Secret Manager"] = True
    except Exception as e:
        fetch_section_succeeded["1.3.3. Get TikTok Ads access token from Google Secret Manager"] = False
        fetch_section_failed.append("1.3.3. Get TikTok Ads access token from Google Secret Manager")
        print(f"❌ [FETCH] Failed to retrieve TikTok access token for {ACCOUNT} from Google Secret Manager due to {e}.")
        logging.error(f"❌ [FETCH] Failed to retrieve TikTok access token for {ACCOUNT} from Google Secret Manager due to {e}.")
        raise RuntimeError(f"❌ [FETCH] Failed to retrieve TikTok access token for {ACCOUNT} from Google Secret Manager due to {e}.")
    ...
    try:
        ...
        print(f"✅ [FETCH] Successfully retrieved TikTok Ads advertiser_id {advertiser_id} from Google Secret Manager.")
        logging.info(f"✅ [FETCH] Successfully retrieved TikTok Ads advertiser_id {advertiser_id} from Google Secret Manager.")
        fetch_section_succeeded["1.3.4. Get TikTok Ads advertiser_id from Google Secret Manager"] = True
    except Exception as e:
        fetch_section_succeeded["1.3.4. Get TikTok Ads advertiser_id from Google Secret Manager"] = False
        fetch_section_failed.append("1.3.4. Get TikTok Ads advertiser_id from Google Secret Manager")
        print(f"❌ [FETCH] Failed to retrieve TikTok Ads access token for {ACCOUNT} from Google Secret Manager due to {e}.")
        logging.error(f"❌ [FETCH] Failed to retrieve TikTok Ads access token for {ACCOUNT} from Google Secret Manager due to {e}.")
        raise RuntimeError(f"❌ [FETCH] Failed to retrieve TikTok Ads access token for {ACCOUNT} from Google Secret Manager due to {e}.")

    # 1.3.5. Make TikTok Ads API call for file/video/ad/search endpoint
    video_search_records = []
    ...
    try:
        ...
        print(f"❌ [FETCH] Failed to retrieve TikTok Ads video creative(s) due to {e}.")
        logging.error(f"❌ [FETCH] Failed to retrieve TikTok Ads video creatives due to {e}.")
        return pd.DataFrame()

    # 1.3.6. Convert TikTok Ads ad creative to Python DataFrame
    try:
        print(f"🔄 [FETCH] Converting TikTok Ads ad creative to Python DataFrame...")
        logging.info(f"🔄 [FETCH] Converting TikTok Ads ad creative to Python DataFrame...")
        fetch_df_flattened = pd.DataFrame(video_search_records)
        print(f"✅ [FETCH] Successfully converted TikTok Ads ad creative to Python DataFrame with {len(fetch_df_flattened)} row(s).")
        logging.info(f"✅ [FETCH] Successfully converted TikTok Ads ad creative to Python DataFrame with {len(fetch_df_flattened)} row(s).")
        fetch_section_succeeded["1.3.6. Convert TikTok Ads ad creative to Python DataFrame"] = True
    except Exception as e:
        fetch_section_succeeded["1.3.6. Convert TikTok Ads ad creative to Python DataFrame"] = False
        fetch_section_failed.append("1.3.6. Convert TikTok Ads ad creative to Python DataFrame")
        print(f"❌ [FETCH] Failed to convert TikTok Ads ad creative to Python DataFrame due to {e}.")
        logging.error(f"❌ [FETCH] Failed to convert TikTok Ads ad creative to Python DataFrame due to {e}.")
        return pd.DataFrame()

    # 1.3.7. Enforce schema for Python DataFrame
    try:
        print(f"🔄 [FETCH] Trigger to enforce schema for {len(fetch_df_flattened)} row(s) of TikTok Ads ad creative...")
        logging.info(f"🔄 [FETCH] Trigger to enforce schema for {len(fetch_df_flattened)} row(s) of TikTok Ads ad creative....")
        fetch_df_enforced = ensure_table_schema(fetch_df_flattened, "fetch_ad_creative")
    except Exception as e:
        print(f"❌ [FETCH] Failed to trigger schema enforcement for TikTok Ads ad creative due to {e}.")
        logging.error(f"❌ [FETCH] Failed to trigger schema enforcement for TikTok Ads ad creative due to {e}.")
        return pd.DataFrame()
    ...
    # 1.3.8. Summarize ingestion result(s)
        ...
        elapsed = round(time.time() - start_time, 2)
        if fetch_section_failed:
            print(f"❌ [FETCH] Failed to completed TikTok Ads ad creative fetching due to unsuccesfull section(s) {', '.join(fetch_section_failed)}.")
            logging.error(f"❌ [FETCH] Failed to completed TikTok Ads ad creative fetching due to unsuccesfull section(s) {', '.join(fetch_section_failed)}.")
            mart_status_def = "failed"
        else:
            fetch_df_final = fetch_df_enforced
            print(f"🏆 [FETCH] Successfully completed TikTok Ads ad creative fetching with {len(fetch_df_final)} row(s) in {elapsed}s.")
            logging.info(f"🏆 [FETCH] Successfully completed TikTok Ads ad creative fetching with {len(fetch_df_final)} row(s) in {elapsed}s.")
            mart_status_def = "success"
            return fetch_df_final
        return {"status": mart_status_def, "elapsed_seconds": elapsed, "failed_sections": fetch_section_failed}
...
def fetch_campaign_insights(start_date: str, end_date: str) -> pd.DataFrame:
    print(f"🚀 [FETCH] Starting to fetch TikTok Ads campaign insights from {start_date} to {end_date}...")
    logging.info(f"🚀 [FETCH] Starting to fetch TikTok Ads campaign insights from {start_date} to {end_date}...")    

    # 2.1.1. Start timing the TikTok Ads campaign insights fetching process
    start_time = time.time()
    fetch_section_succeeded = {}
    fetch_section_failed = [] 
    ...
    try:
        ...
        print(f"✅ [FETCH] Successfully initialized Google Secret Manager client for Google Cloud project {PROJECT}.")
        logging.info(f"✅ [FETCH] Successfully initialized Google Secret Manager client for Google Cloud project {PROJECT}.")
        fetch_section_succeeded["2.1.2 Initialize Google Secret Manager client"] = True
    except Exception as e:
        fetch_section_succeeded["2.1.2 Initialize Google Secret Manager client"] = False
        fetch_section_failed.append("2.1.2 Initialize Google Secret Manager client")
        print(f"❌ [FETCH] Failed to initialize Google Secret Manager client for Google Cloud Platform project {PROJECT} due to {e}.")
        logging.error(f"❌ [FETCH] Failed to initialize Google Secret Manager client for Google Cloud Platform project {PROJECT} due to {e}.")
        raise RuntimeError(f"❌ [FETCH] Failed to initialize Google Secret Manager client for Google Cloud Platform project {PROJECT} due to {e}.") from e
    ...
    try: 
        ...
        print(f"✅ [FETCH] Successfully retrieved TikTok access token for account {ACCOUNT} from Google Secret Manager.")
        logging.info(f"✅ [FETCH] Successfully retrieved TikTok access token for account {ACCOUNT} from Google Secret Manager.")
        fetch_section_succeeded["2.1.3. Get TikTok Ads access token from Google Secret Manager"] = True
    except Exception as e:
        fetch_section_succeeded["2.1.3. Get TikTok Ads access token from Google Secret Manager"] = False
        fetch_section_failed.append("2.1.3. Get TikTok Ads access token from Google Secret Manager")
        print(f"❌ [FETCH] Failed to retrieve TikTok access token for {ACCOUNT} from Google Secret Manager due to {e}.")
        logging.error(f"❌ [FETCH] Failed to retrieve TikTok access token for {ACCOUNT} from Google Secret Manager due to {e}.")
        raise RuntimeError(f"❌ [FETCH] Failed to retrieve TikTok access token for {ACCOUNT} from Google Secret Manager due to {e}.")
    ...
    try:
        ...
        print(f"✅ [FETCH] Successfully retrieved TikTok Ads advertiser_id {advertiser_id} from Google Secret Manager.")
        logging.info(f"✅ [FETCH] Successfully retrieved TikTok Ads advertiser_id {advertiser_id} from Google Secret Manager.")
        fetch_section_succeeded["2.1.4. Get TikTok Ads advertiser_id from Google Secret Manager"] = True
    except Exception as e:
        fetch_section_succeeded["2.1.4. Get TikTok Ads advertiser_id from Google Secret Manager"] = False
        fetch_section_failed.append("2.1.4. Get TikTok Ads advertiser_id from Google Secret Manager")
        print(f"❌ [FETCH] Failed to retrieve TikTok Ads access token for {ACCOUNT} from Google Secret Manager due to {e}.")
        logging.error(f"❌ [FETCH] Failed to retrieve TikTok Ads access token for {ACCOUNT} from Google Secret Manager due to {e}.")
        raise RuntimeError(f"❌ [FETCH] Failed to retrieve TikTok Ads access token for {ACCOUNT} from Google Secret Manager due to {e}.")

    # 2.1.5. Make TikTok Ads API call for BASIC report type
    campaign_report_records = []
    ...
    try:
        ...
        fetch_section_succeeded["2.1.7. Trigger to enforce schema for TikTok Ads campaign insights"] = False
        fetch_section_failed.append("2.1.7. Trigger to enforce schema for TikTok Ads campaign insights")
        print(f"❌ [FETCH] Failed to trigger schema enforcement for TikTok Ads campaign insights due to {e}.")
        logging.error(f"❌ [FETCH] Failed to trigger schema enforcement for TikTok Ads campaign insights due to {e}.")
        return pd.DataFrame()

    # 2.1.8. Summarize ingestion result(s)
    finally:
        elapsed = round(time.time() - start_time, 2)
        if fetch_section_failed:
            print(f"❌ [FETCH] Failed to completed TikTok Ads campaign insights fetching due to unsuccesfull section(s) {', '.join(fetch_section_failed)}.")
            logging.error(f"❌ [FETCH] Failed to completed TikTok Ads campaign insights fetching due to unsuccesfull section(s) {', '.join(fetch_section_failed)}.")
            mart_status_def = "failed"
        else:
            fetch_df_final = fetch_df_enforced
            print(f"🏆 [FETCH] Successfully completed TikTok Ads campaign insights fetching with {len(fetch_df_final)} row(s) in {elapsed}s.")
            logging.info(f"🏆 [FETCH] Successfully completed TikTok Ads campaign insights fetching with {len(fetch_df_final)} row(s) in {elapsed}s.")
            mart_status_def = "success"
            return fetch_df_final
        return {"status": mart_status_def, "elapsed_seconds": elapsed, "failed_sections": fetch_section_failed}
...
def fetch_ad_insights(start_date: str, end_date: str) -> pd.DataFrame:
    print(f"🚀 [FETCH] Starting to fetch TikTok Ads ad insights from {start_date} to {end_date}...")
    logging.info(f"🚀 [FETCH] Starting to fetch TikTok Ads ad insights from {start_date} to {end_date}...")    

    # 2.2.1. Start timing the TikTok Ads ad insights fetching process
    start_time = time.time()
    fetch_section_succeeded = {}
    fetch_section_failed = [] 
    ...
    try:
        ...
        print(f"✅ [FETCH] Successfully initialized Google Secret Manager client for Google Cloud project {PROJECT}.")
        logging.info(f"✅ [FETCH] Successfully initialized Google Secret Manager client for Google Cloud project {PROJECT}.")
        fetch_section_succeeded["2.2.2 Initialize Google Secret Manager client"] = True
    except Exception as e:
        fetch_section_succeeded["2.2.2 Initialize Google Secret Manager client"] = False
        fetch_section_failed.append("2.2.2 Initialize Google Secret Manager client")
        print(f"❌ [FETCH] Failed to initialize Google Secret Manager client for Google Cloud Platform project {PROJECT} due to {e}.")
        logging.error(f"❌ [FETCH] Failed to initialize Google Secret Manager client for Google Cloud Platform project {PROJECT} due to {e}.")
        raise RuntimeError(f"❌ [FETCH] Failed to initialize Google Secret Manager client for Google Cloud Platform project {PROJECT} due to {e}.") from e
    ...
    try: 
        ...
        print(f"✅ [FETCH] Successfully retrieved TikTok access token for account {ACCOUNT} from Google Secret Manager.")
        logging.info(f"✅ [FETCH] Successfully retrieved TikTok access token for account {ACCOUNT} from Google Secret Manager.")
        fetch_section_succeeded["2.2.3. Get TikTok Ads access token from Google Secret Manager"] = True
    except Exception as e:
        fetch_section_succeeded["2.2.3. Get TikTok Ads access token from Google Secret Manager"] = False
        fetch_section_failed.append("2.2.3. Get TikTok Ads access token from Google Secret Manager")
        print(f"❌ [FETCH] Failed to retrieve TikTok access token for {ACCOUNT} from Google Secret Manager due to {e}.")
        logging.error(f"❌ [FETCH] Failed to retrieve TikTok access token for {ACCOUNT} from Google Secret Manager due to {e}.")
        raise RuntimeError(f"❌ [FETCH] Failed to retrieve TikTok access token for {ACCOUNT} from Google Secret Manager due to {e}.")
    ...
    try:
        ...
        print(f"✅ [FETCH] Successfully retrieved TikTok Ads advertiser_id {advertiser_id} from Google Secret Manager.")
        logging.info(f"✅ [FETCH] Successfully retrieved TikTok Ads advertiser_id {advertiser_id} from Google Secret Manager.")
        fetch_section_succeeded["2.2.4. Get TikTok Ads advertiser_id from Google Secret Manager"] = True
    except Exception as e:
        fetch_section_succeeded["2.2.4. Get TikTok Ads advertiser_id from Google Secret Manager"] = False
        fetch_section_failed.append("2.2.4. Get TikTok Ads advertiser_id from Google Secret Manager")
        print(f"❌ [FETCH] Failed to retrieve TikTok Ads access token for {ACCOUNT} from Google Secret Manager due to {e}.")
        logging.error(f"❌ [FETCH] Failed to retrieve TikTok Ads access token for {ACCOUNT} from Google Secret Manager due to {e}.")
        raise RuntimeError(f"❌ [FETCH] Failed to retrieve TikTok Ads access token for {ACCOUNT} from Google Secret Manager due to {e}.")
    ...
    for attempt in range(2):
        try:
            ...
            for record in ad_report_records:
                ...
                row.update(record.get("dimensions", {}))
                row.update(record.get("metrics", {}))
                row["advertiser_id"] = ad_report_params["advertiser_id"]
                ad_records_flattened.append(row)
            fetch_df_flattened = pd.DataFrame(ad_records_flattened)
            print(f"✅ [FETCH] Successfully retrieved {len(fetch_df_flattened)} rows of TikTok Ads ad-level insights with BASIC report_type.")
            logging.info(f"✅ [FETCH] Successfully retrieved {len(fetch_df_flattened)} rows of TikTok Ads ad-level insights with BASIC report_type.")
            break
        except Exception as e:
            if attempt < 1:
    ...
    try:
        print(f"🔄 [FETCH] Trigger to enforce schema for {len(fetch_df_flattened)} row(s) of TikTok Ads ad insights...")
        logging.info(f"🔄 [FETCH] Trigger to enforce schema for {len(fetch_df_flattened)} row(s) of TikTok Ads ad insights....")
        fetch_df_enforced = ensure_table_schema(fetch_df_flattened, "fetch_ad_insights")
    except Exception as e:
        fetch_section_succeeded["2.2.6. Trigger to enforce schema for TikTok Ads ad insights"] = False
        fetch_section_failed.append("2.2.6. Trigger to enforce schema for TikTok Ads ad insights")
        print(f"❌ [FETCH] Failed to trigger schema enforcement for TikTok Ads ad insights due to {e}.")
        logging.error(f"❌ [FETCH] Failed to trigger schema enforcement for TikTok Ads adinsights due to {e}.")
        return pd.DataFrame()
    ...
    # 2.2.7. Summarize ingestion result(s)
        ...
        elapsed = round(time.time() - start_time, 2)
        if fetch_section_failed:
            print(f"❌ [FETCH] Failed to completed TikTok Ads ad insights fetching due to unsuccesfull section(s) {', '.join(fetch_section_failed)}.")
            logging.error(f"❌ [FETCH] Failed to completed TikTok Ads ad insights fetching due to unsuccesfull section(s) {', '.join(fetch_section_failed)}.")
            mart_status_def = "failed"
        else:
            fetch_df_final = fetch_df_enforced
            print(f"🏆 [FETCH] Successfully completed TikTok Ads ad insights fetching with {len(fetch_df_final)} row(s) in {elapsed}s.")
            logging.info(f"🏆 [FETCH] Successfully completed TikTok Ads ad insights fetching with {len(fetch_df_final)} row(s) in {elapsed}s.")
            mart_status_def = "success"
            return fetch_df_final
        return {"status": mart_status_def, "elapsed_seconds": elapsed, "failed_sections": fetch_section_failed}