...
def ingest_campaign_metadata(campaign_id_list: list) -> pd.DataFrame:
    ...
    # 1.1.2. Prepare table_id for TikTok Ads campaign metadata ingestion
    ...
    try:
        ...
        ingest_section_succeeded["1.1.5. Initialize Google BigQuery client"] = True
    except Exception as e:
        ingest_section_succeeded["1.1.5. Initialize Google BigQuery client"] = False
        ingest_section_failed.append("1.1.5. Initialize Google BigQuery client")
    ...
    # 1.1.6. Delete existing row(s) or create new table if not exist
    try:
...
def ingest_ad_metadata(ad_id_list: list) -> pd.DataFrame:
    print(f"🚀 [INGEST] Starting to ingest TikTok Ads ad metadat for {len(ad_id_list)} ad_id(s)...")
    logging.info(f"🚀 [INGEST] Starting to ingest TikTok Ads ad metadat for {len(ad_id_list)} ad_id(s)...")

    # 1.2.1. Validate input for TikTok Ads ad metadata ingestion
    if not ad_id_list:
    ...
    # 1.2.2. Trigger to fetch TikTok ad metadata
        try:
    ...
    # 1.2.4. Enforce schema for TikTok ad metadata
        try:
    ...
    # 1.2.6. Upload TikTok Ads ad metadata to Google BigQuery
        try:
    ...
    # 1.1.7. Summarize ingestion result(s)
        ingest_df_final = ingest_df_deduplicated
        print(f"🏆 [INGEST] Successfully completed TikTok Ads ad metadata ingestion with {len(ingest_df_final)} row(s).")
        logging.info(f"🏆 [INGEST] Successfully completed TikTok Ads ad metadata ingestion with {len(ingest_df_final)} row(s).")
        return ingest_df_final
    except Exception as e:
...
def ingest_ad_creative() -> pd.DataFrame:
    ...
    # 1.3.6. Upload TikTok Ads ad creative to Google BigQuery
        try:
    ...
    # 1.3.7. Summarize ingestion result(s)
        ingest_df_final = ingest_df_deduplicated
        print(f"🏆 [INGEST] Successfully completed TikTok Ads ad creative ingestion with {len(ingest_df_final)} row(s).")
        logging.info(f"🏆 [INGEST] Successfully completed TikTok Ads ad creative ingestion with {len(ingest_df_final)} row(s).")
        return ingest_df_final
    except Exception as e:
        print(f"❌ [INGEST] Failed to ingest TikTok Ads ad creative due to {e}.")
        logging.error(f"❌ [INGEST] Failed to ingest TikTok Ads ad creative due to {e}.")
        return pd.DataFrame()
...
def ingest_campaign_insights(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    # 2.1.1. Trigger to fetch TikTok Ads campaign insights
        try:
            ...
            if ingest_df_fetched.empty:
                ...
                return pd.DataFrame()
            print(f"✅ [INGEST] Successfully fetched TikTok Ads campaign insights from {start_date} to {end_date} with {len(ingest_df_fetched)} row(s).")
            logging.info(f"✅ [INGEST] Successfully fetched TikTok Ads campaign insights from {start_date} to {end_date} with {len(ingest_df_fetched)} row(s).")
        except Exception as e:
            print(f"❌ [INGEST] Failed to trigger TikTok Ads campaign insights fetching from {start_date} to {end_date} due to {e}.")
    ...
    # 2.1.5. Delete existing row(s) or create new table if not exist
        try:
            ingest_df_deduplicated = ingest_df_enforced.drop_duplicates()          
            try:
                print(f"🔍 [INGEST] Initializing Google BigQuery client for Google Cloud Platform project {PROJECT}...")
    ...
    # 2.1.6. Upload TikTok Ads campaign insights to Google BigQuery
        try:
    ...
    # 2.1.7. Summarize ingestion result(s)
        ingest_df_final = ingest_df_deduplicated
        print(f"🏆 [INGEST] Successfully completed TikTok Ads campaign insights ingestion from {start_date} to {end_date} with {len(ingest_df_final)} row(s).")
        logging.info(f"🏆 [INGEST] Successfully completed TikTok Ads campaign insights ingestion from {start_date} to {end_date} with {len(ingest_df_final)} row(s).")
        return ingest_df_final
    except Exception as e:
        print(f"❌ [INGEST] Failed to ingest TikTok Ads campaign insights from {start_date} to {end_date} due to {e}.")
        logging.error(f"❌ [INGEST] Failed to ingest TikTok Ads campaign insights from {start_date} to {end_date} due to {e}.")
        return pd.DataFrame()
...
def ingest_ad_insights(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    # 2.2.1. Trigger to fetch ad insights for TikTok Ads
        try:
            ...
            if ingest_df_fetched.empty:
                ...
                return pd.DataFrame()
            print(f"✅ [INGEST] Successfully fetched TikTok Ads ad insights from {start_date} to {end_date} with {len(ingest_df_fetched)} row(s).")
            logging.info(f"✅ [INGEST] Successfully fetched TikTok Ads ad insights from {start_date} to {end_date} with {len(ingest_df_fetched)} row(s).")
        except Exception as e:
            print(f"❌ [INGEST] Failed to trigger TikTok Ads ad insights fetching from {start_date} to {end_date} due to {e}.")
            logging.error(f"❌ [INGEST] Failed to trigger TikTok Ads ad insights fetching from {start_date} to {end_date} due to {e}.")
            return pd.DataFrame()

    # 2.2.2. Prepare table_id for TikTok Ads ad insights
        first_date = pd.to_datetime(ingest_df_fetched["stat_time_day"].dropna().iloc[0])
        y, m = first_date.year, first_date.month
        raw_dataset = f"{COMPANY}_dataset_{PLATFORM}_api_raw"
    ...
    # 2.2.5. Delete existing row(s) or create new table if not exist
        try:
            ingest_df_deduplicated = ingest_df_enforced.drop_duplicates()
            try:
                print(f"🔍 [INGEST] Initializing Google BigQuery client for Google Cloud Platform project {PROJECT}...")
    ...
    # 2.2.6. Upload TikTok Ads ad insights to Google BigQuery
        try:
    ...
    # 2.2.7. Summarize ingestion result(s)
        ingest_df_final = ingest_df_deduplicated
        print(f"🏆 [INGEST] Successfully completed TikTok Ads ad insights ingestion from {start_date} to {end_date} with {len(ingest_df_final)} row(s).")
        logging.info(f"🏆 [INGEST] Successfully completed TikTok Ads ad insights ingestion from {start_date} to {end_date} with {len(ingest_df_final)} row(s).")
        return ingest_df_final
    except Exception as e:
        print(f"❌ [INGEST] Failed to ingest TikTok Ads campaign insights from {start_date} to {end_date} due to {e}.")
        logging.error(f"❌ [INGEST] Failed to ingest TikTok Ads campaign insights from {start_date} to {end_date} due to {e}.")
