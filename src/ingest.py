...
def ingest_ad_metadata(ad_id_list: list) -> pd.DataFrame:
    ...
    # 1.2.2. Trigger to fetch TikTok ad metadata
        try:
            ...
            print(f"❌ [INGEST] Failed to trigger TikTok Ads ad metadata fetch due to {e}.")
            logging.error(f"❌ [INGEST] Failed to trigger TikTok Ads ad metadata fetch due to {e}.")
            return pd.DataFrame()

    # 1.2.3. Prepare table_id for TikTok Ads ad metadata
        raw_dataset = f"{COMPANY}_dataset_{PLATFORM}_api_raw"
    ...
    # 1.2.6. Upload TikTok Ads ad metadata to Google BigQuery
        try:
            print(f"🔍 [INGEST] Uploading {len(ingest_df_deduplicated)} row(s) of TikTok Ads ad metadata {table_id}...")
            logging.info(f"🔍 [INGEST] Uploading {len(ingest_df_deduplicated)} row(s) of TikTok Ads ad metadata {table_id}...")
            job_config = bigquery.LoadJobConfig(write_disposition="WRITE_APPEND")
            google_bigquery_client.load_table_from_dataframe(ingest_df_deduplicated, table_id, job_config=job_config).result()
            print(f"✅ [INGEST] Successfully uploaded {len(ingest_df_deduplicated)} row(s) of TikTok Ads ad metadata table {table_id}.")
            logging.info(f"✅ [INGEST] Successfully uploaded {len(ingest_df_deduplicated)} row(s) of TikTok Ads ad metadata table {table_id}.")
        except Exception as e:
