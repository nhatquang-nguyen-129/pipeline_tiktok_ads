...
def ingest_campaign_metadata(ingest_ids_campaign: list) -> pd.DataFrame:
    ...
    print(f"🔍 [INGEST] Proceeding to ingest raw TikTok Ads campaign metadata at {time.strftime('%Y-%m-%d %H:%M:%S')}...")
...
def ingest_ad_metadata(ingest_ids_ad: list) -> pd.DataFrame:
    ...
    # 1.2.5. Trigger to enforce schema for TikTok Ads ad metadata
        ...
        try:
            ...
            ingest_df_enforced = ingest_results_enforced["schema_df_final"]   
            if ingest_status_enforced == "schema_succeed_all":
                print(f"✅ [INGEST] Successfully triggered schema enforcement for TikTok Ads ad metadata with {ingest_summary_enforced['schema_rows_output']}/{ingest_summary_enforced['schema_rows_input']} enforced row(s) in {ingest_summary_enforced['schema_time_elapsed']}s.")
                logging.info(f"✅ [INGEST] Successfully triggered schema enforcement for TikTok Ads ad metadata with {ingest_summary_enforced['schema_rows_output']}/{ingest_summary_enforced['schema_rows_input']} enforced row(s) in {ingest_summary_enforced['schema_time_elapsed']}s.")
                ingest_sections_status[ingest_section_name] = "succeed"
            else:
                ingest_sections_status[ingest_section_name] = "failed"
                print(f"❌ [INGEST] Failed to trigger schema enforcement for TikTOk Ads ad metadata with {ingest_summary_enforced['schema_rows_output']}/{ingest_summary_enforced['schema_rows_input']} enforced row(s) due to failed sections "f"{', '.join(ingest_summary_enforced['schema_sections_failed'])}.")
                logging.error(f"❌ [INGEST] Failed to retrieve schema enforcement for TikTok Ads ad metadata with {ingest_summary_enforced['schema_rows_output']}/{ingest_summary_enforced['schema_rows_input']} enforced row(s) due to failed sections "f"{', '.join(ingest_summary_enforced['schema_sections_failed'])}.")
        finally:
            ingest_sections_time[ingest_section_name] = round(time.time() - ingest_section_start, 2)
        ...
        try:
            ...
            if not ingest_table_existed:
                ...
                try:    
                    ...
                    table_metadata_defined = google_bigquery_client.create_table(table_configuration_defined)
                    print(f"✅ [INGEST] Successfully created TikTok Ads ad metadata table actual name {table_metadata_defined.full_table_id} with partition on {table_partition_effective} and cluster on {table_clusters_filtered}.")
                    logging.info(f"✅ [INGEST] Successfully created TikTok Ads ad metadata table actual name {table_metadata_defined.full_table_id} with partition on {table_partition_effective} and cluster on {table_clusters_filtered}.")
                except Exception as e:
...
def ingest_ad_creative() -> pd.DataFrame:
    ...
    # 1.3.3. Prepare Google BigQuery table_id for ingestion
        ingest_section_name = "[INGEST] Prepare Google BigQuery table_id for ingestion"
        ingest_section_start = time.time()    
        try:        
            raw_dataset = f"{COMPANY}_dataset_{PLATFORM}_api_raw"
    ...
    # 1.3.4. Trigger to enforce schema for TikTok Ads ad creative
        ...
        try:
            ...
            ingest_df_enforced = ingest_results_enforced["schema_df_final"]   
            if ingest_status_enforced == "schema_succeed_all":
                print(f"✅ [INGEST] Successfully triggered schema enforcement for TikTok Ads ad creative with {ingest_summary_enforced['schema_rows_output']}/{ingest_summary_enforced['schema_rows_input']} enforced row(s) in {ingest_summary_enforced['schema_time_elapsed']}s.")
                logging.info(f"✅ [INGEST] Successfully triggered schema enforcement for TikTok Ads ad creative with {ingest_summary_enforced['schema_rows_output']}/{ingest_summary_enforced['schema_rows_input']} enforced row(s) in {ingest_summary_enforced['schema_time_elapsed']}s.")
                ingest_sections_status[ingest_section_name] = "succeed"
            else:
                ingest_sections_status[ingest_section_name] = "failed"
                print(f"❌ [INGEST] Failed to trigger schema enforcement for TikTok Ads ad creative with {ingest_summary_enforced['schema_rows_output']}/{ingest_summary_enforced['schema_rows_input']} enforced row(s) due to failed sections "f"{', '.join(ingest_summary_enforced['schema_sections_failed'])}.")
                logging.error(f"❌ [INGEST] Failed to trigger schema enforcement for TikTok Ads ad creative with {ingest_summary_enforced['schema_rows_output']}/{ingest_summary_enforced['schema_rows_input']} enforced row(s) due to failed sections "f"{', '.join(ingest_summary_enforced['schema_sections_failed'])}.")
        finally:
            ingest_sections_time[ingest_section_name] = round(time.time() - ingest_section_start, 2) 
        ...
        try:
            ...
            if not ingest_table_existed:
                ...
                try:    
                    ...
                    table_metadata_defined = google_bigquery_client.create_table(table_configuration_defined)
                    print(f"✅ [INGEST] Successfully created TikTok Ads ad creative table actual name {table_metadata_defined.full_table_id} with partition on {table_partition_effective} and cluster on {table_clusters_filtered}.")
                    logging.info(f"✅ [INGEST] Successfully created TikTok Ads ad creative table actual name {table_metadata_defined.full_table_id} with partition on {table_partition_effective} and cluster on {table_clusters_filtered}.")
                except Exception as e:
...
def ingest_campaign_insights(start_date: str, end_date: str,) -> pd.DataFrame:  
    ...
    # 2.1.3. Trigger to fetch TikTok Ads campaign insights
        ...
        for ingest_date_separated in ingest_date_list:           
            ...
            ingest_section_start = time.time()
            try:
                print(f"🔁 [INGEST] Triggering to fetch raw TikTok Ads campaigns insights for {ingest_date_separated}...")
                logging.info(f"🔁 [INGEST] Triggering to fetch raw TikTok Ads campaigns insights for {ingest_date_separated}...")
    ...
    # 2.1.5. Prepare Google BigQuery table_id for ingestion
            ...
            try:
                ...
                ingest_sections_status[ingest_section_name] = "succeed"
                print(f"🔍 [INGEST] Proceeding to ingest raw TikTok Ads campaign insights for {ingest_date_separated} to Google BigQuery table_id {raw_table_campaign}...")
                logging.info(f"🔍 [INGEST] Proceeding to ingest raw TikTok Ads campaign insights for {ingest_date_separated} to Google BigQuery table_id {raw_table_campaign}...")
            finally:
                ingest_loops_time[ingest_section_name] += round(time.time() - ingest_section_start, 2)     
    ...
    # 2.1.6. Delete existing row(s) or create new table if not exist
            ...
            try:
                ...
                table_schemas_defined = []
                try:
                    print(f"🔍 [INGEST] Checking raw TikTok Ads campaign insights table {raw_table_campaign} existence...")
                    logging.info(f"🔍 [INGEST] Checking raw TikTok Ads campaign insights table {raw_table_campaign} existence...")
                    google_bigquery_client.get_table(raw_table_campaign)
                    ingest_table_existed = True
                except NotFound:
                    ingest_table_existed = False
                except Exception as e:
                    print(f"❌ [INGEST] Failed to check raw TikTok Ads campaign insights table {raw_table_campaign} existence due to {e}.")
                    logging.error(f"❌ [INGEST] Failed to check raw TikTok Ads campaign insights table {raw_table_campaign} existence due to {e}.")
                if not ingest_table_existed:
                    ...
                    if table_clusters_filtered:
                        table_configuration_defined.clustering_fields = table_clusters_filtered
                    try:
                        print(f"🔍 [INGEST] Creating raw TikTok Ads campaign insights table {raw_table_campaign} with partition on {table_partition_effective} and cluster on {table_clusters_filtered}...")
                        logging.info(f"🔍 [INGEST] Creating raw TikTok Ads campaign insights table {raw_table_campaign} with partition on {table_partition_effective} and cluster on {table_clusters_filtered}...")
                        table_metadata_defined = google_bigquery_client.create_table(table_configuration_defined)
                        print(f"✅ [INGEST] Successfully created raw TikTok Ads campaign insights table {table_metadata_defined.full_table_id} with partition on {table_partition_effective} and cluster on {table_clusters_filtered}.")
                        logging.info(f"✅ [INGEST] Successfully created raw TikTok Ads campaign insights table {table_metadata_defined.full_table_id} with partition on {table_partition_effective} and cluster on {table_clusters_filtered}.")
                    except Exception as e:
                        print(f"❌ [INGEST] Failed to create raw TikTok Ads campaign insights table {raw_table_campaign} due to {e}.")
                        logging.error(f"❌ [INGEST] Failed to create raw TikTok Ads campaign insights table {raw_table_campaign} due to {e}.")
                else:
                    ...
                    if ingest_dates_overlapped:
                        ...
                        for ingest_date_overlapped in ingest_dates_overlapped:
                            ...
                            try:
                                ...
                                print(f"✅ [INGEST] Successfully deleted {ingest_rows_deleted} existing row(s) of raw TikTok Ads campaign insights for {ingest_date_overlapped} in Google BigQuery table {raw_table_campaign}.")
                                logging.info(f"✅ [INGEST] Successfully deleted {ingest_rows_deleted} existing row(s) of raw TikTok Ads campaign insights for {ingest_date_overlapped} in Google BigQuery table {raw_table_campaign}.")
                            except Exception as e:
                                print(f"❌ [INGEST] Failed to delete existing row(s) of raw TikTok Ads campaign insights for {ingest_date_overlapped} in Google BigQuery table {raw_table_campaign} due to {e}.")
                                logging.error(f"❌ [INGEST] Failed to delete existing row(s) of raw TikTok Ads campaign insights for {ingest_date_overlapped} in Google BigQuery table {raw_table_campaign} due to {e}.")
                    else:
                        print(f"⚠️ [INGEST] No overlapping date(s) of raw TikTok Ads campaign insights found in Google BigQuery {raw_table_campaign} table then deletion is skipped.")
                        logging.info(f"⚠️ [INGEST] No overlapping date(s) of raw TikTok Ads campaign insights found in Google BigQuery {raw_table_campaign} table then deletion is skipped.")
                ingest_sections_status[ingest_section_name] = "succeed"
            except Exception as e:
                ingest_sections_status[ingest_section_name] = "failed"
                print(f"❌ [INGEST] Failed to delete existing row(s) or create new table {raw_table_campaign} if it not exist for raw TikTok Ads campaign insights due to {e}.")
                logging.error(f"❌ [INGEST] Failed to delete existing row(s) or create new table {raw_table_campaign} if it not exist for raw TikTok Ads campaign insights due to {e}.")
            finally:
                ingest_loops_time[ingest_section_name] += round(time.time() - ingest_section_start, 2)
    ...
    # 2.1.7. Upload TikTok Ads campaign insights to Google BigQuery
            ...
            ingest_section_start = time.time()
            try:
                print(f"🔍 [INGEST] Uploading {len(ingest_df_deduplicated)} row(s) of raw TikTok Ads campaign insights to Google BigQuery table {raw_table_campaign}...")
                logging.info(f"🔍 [INGEST] Uploading {len(ingest_df_deduplicated)} row(s) of raw TikTok Ads campaign insights to Google BigQuery table {raw_table_campaign}...")
                job_config = bigquery.LoadJobConfig(write_disposition="WRITE_APPEND")
                ...
                ingest_sections_status[ingest_section_name] = "succeed"
                print(f"✅ [INGEST] Successfully uploaded {len(ingest_df_deduplicated)} row(s) of raw TikTok Ads campaign insights to Google BigQuery table {raw_table_campaign}.")
                logging.info(f"✅ [INGEST] Successfully uploaded {len(ingest_df_deduplicated)} row(s) of raw TikTok Ads campaign insights to Google BigQuery table {raw_table_campaign}.")
            except Exception as e:
                ingest_sections_status[ingest_section_name] = "failed"
                print(f"❌ [INGEST] Failed to upload {len(ingest_df_deduplicated)} row(s) of raw TikTok Ads campaign insights to Google BigQuery table {raw_table_campaign} due to {e}.")
                logging.error(f"❌ [INGEST] Failed to upload {len(ingest_df_deduplicated)} row(s) of raw TikTok Ads campaign insights to Google BigQuery table {raw_table_campaign} due to {e}.")
            finally:
                ingest_loops_time[ingest_section_name] += round(time.time() - ingest_section_start, 2) 

    # 2.1.8. Summarize ingestion results for TikTok Ads campaign insights
    finally:
        ingest_time_elapsed = round(time.time() - ingest_time_start, 2)
        ...
        if len(ingest_dates_uploaded) == 0:
            print(f"❌ [INGEST] Failed to complete TikTok Ads campaign insights ingestion from {start_date} to {end_date} with {ingest_dates_output}/{ingest_dates_input} ingested day(s) and {ingest_rows_output} ingested row(s) in {ingest_time_elapsed}s.")
            logging.error(f"❌ [INGEST] Failed to complete TikTok Ads campaign insights ingestion from {start_date} to {end_date} with {ingest_dates_output}/{ingest_dates_input} ingested day(s) and {ingest_rows_output} ingested row(s) in {ingest_time_elapsed}s.")
            ingest_status_final = "ingest_failed_all"
        elif ingest_dates_failed > 0:
            print(f"⚠️ [INGEST] Partially completed TikTok Ads campaign insights ingestion from {start_date} to {end_date} with {ingest_dates_output}/{ingest_dates_input} ingested day(s) and {ingest_rows_output} ingested row(s) in {ingest_time_elapsed}s.")
            logging.warning(f"⚠️ [INGEST] Partially completed TikTok Ads campaign insights ingestion from {start_date} to {end_date} with {ingest_dates_output}/{ingest_dates_input} ingested day(s) and {ingest_rows_output} ingested row(s) in {ingest_time_elapsed}s.")
            ingest_status_final = "ingest_succeed_partial"
        else:
            print(f"🏆 [INGEST] Successfully completed TikTok Ads campaign insights ingestion from {start_date} to {end_date} with {ingest_dates_output}/{ingest_dates_input} ingested day(s) and {ingest_rows_output} ingested row(s) in {ingest_time_elapsed}s.")
            logging.info(f"🏆 [INGEST] Successfully completed TikTok Ads campaign insights ingestion from {start_date} to {end_date} with {ingest_dates_output}/{ingest_dates_input} ingested day(s) and {ingest_rows_output} ingested row(s) in {ingest_time_elapsed}s.")
            ingest_status_final = "ingest_succeed_all"
        ingest_results_final = {
            "ingest_df_final": ingest_df_final,
            "ingest_status_final": ingest_status_final,
...
def ingest_ad_insights(start_date: str, end_date: str,) -> pd.DataFrame:  
    print(f"🚀 [INGEST] Starting to ingest raw TikTok Ads ad insights from {start_date} to {end_date}...")
    logging.info(f"🚀 [INGEST] Starting to ingest raw TikTok Ads ad insights from {start_date} to {end_date}...")
    ...
    print(f"🔍 [INGEST] Proceeding to ingest raw TikTok Ads ad insights from {start_date} to {end_date} at {time.strftime('%Y-%m-%d %H:%M:%S')}...")
    logging.info(f"🔍 [INGEST] Proceeding to ingest raw TikTok Ads ad insights from {start_date} to {end_date} at {time.strftime('%Y-%m-%d %H:%M:%S')}...")
    ...
    # 2.2.3. Trigger to fetch TikTok Ads ad insights
        ...
        for ingest_date_separated in ingest_date_list:
            ...
            ingest_section_start = time.time()
            try:
                print(f"🔁 [INGEST] Triggering to fetch raw TikTok Ads ad insights for {ingest_date_separated}...")
                logging.info(f"🔁 [INGEST] Triggering to fetch raw TikTok Ads ad insights for {ingest_date_separated}...")
    ...
    # 2.2.4. Trigger to enforce schema for TikTok Ads ad insights
            ...
            ingest_section_start = time.time()
            try:
                print(f"🔁 [INGEST] Triggering to enforce schema for raw TikTok Ads ad insights for {ingest_date_separated} with {len(ingest_df_fetched)} row(s)...")
                logging.info(f"🔁 [INGEST] Triggering to enforce schema for raw TikTok Ads ad insights for {ingest_date_separated} with {len(ingest_df_fetched)} row(s)...")
                ingest_results_enforced = enforce_table_schema(schema_df_input=ingest_df_fetched,schema_type_mapping="ingest_ad_insights")
                ingest_df_enforced = ingest_results_enforced["schema_df_final"]
                ingest_status_enforced = ingest_results_enforced["schema_status_final"]
                ingest_summary_enforced = ingest_results_enforced["schema_summary_final"]              
                if ingest_status_enforced == "schema_succeed_all":
                    print(f"✅ [INGEST] Successfully triggered raw TikTok Ads ad insights schema enforcement for {ingest_date_separated} with {ingest_summary_enforced['schema_rows_output']}/{ingest_summary_enforced['schema_rows_input']} enforced row(s) in {ingest_summary_enforced['schema_time_elapsed']}s.")
                    ...
                    ingest_sections_status[ingest_section_name] = "failed"
                    print(f"❌ [INGEST] Failed to trigger raw TikTok Ads ad insights schema enforcement for {ingest_date_separated} with {ingest_summary_enforced['schema_rows_output']}/{ingest_summary_enforced['schema_rows_input']} enforced row(s) due to failed section(s) {', '.join(ingest_summary_enforced['schema_sections_failed']) if ingest_summary_enforced['schema_sections_failed'] else 'unknown error'} in {ingest_summary_enforced['schema_time_elapsed']}s.")
                    logging.error(f"❌ [INGEST] Failed to trigger raw TikTok Ads ad insights schema enforcement for {ingest_date_separated} with {ingest_summary_enforced['schema_rows_output']}/{ingest_summary_enforced['schema_rows_input']} enforced row(s) due to failed section(s) {', '.join(ingest_summary_enforced['schema_sections_failed']) if ingest_summary_enforced['schema_sections_failed'] else 'unknown error'} in {ingest_summary_enforced['schema_time_elapsed']}s.")
            finally:
                ingest_loops_time[ingest_section_name] += round(time.time() - ingest_section_start, 2)
    ...
    # 2.2.5. Prepare Google BigQuery table_id for ingestion
            ...
            try:
                ...
                ingest_sections_status[ingest_section_name] = "succeed"
                print(f"🔍 [INGEST] Proceeding to ingest raw TikTok Ads ad insights for {ingest_date_separated} to Google BigQuery table_id {raw_table_ad}...")
                logging.info(f"🔍 [INGEST] Proceeding to ingest raw TikTok Ads ad insights for {ingest_date_separated} to Google BigQuery table_id {raw_table_ad}...")
            finally:
                ingest_loops_time[ingest_section_name] += round(time.time() - ingest_section_start, 2)
    ...
    # 2.2.6. Delete existing row(s) or create new table if not exist
            ...
            try:
                ...
                table_schemas_defined = []
                try:
                    print(f"🔍 [INGEST] Checking raw TikTok Ads ad insights table {raw_table_ad} existence...")
                    logging.info(f"🔍 [INGEST] Checking raw TikTok Ads ad insights table {raw_table_ad} existence...")
                    google_bigquery_client.get_table(raw_table_ad)
                    ingest_table_existed = True
                except NotFound:
                    ingest_table_existed = False
                except Exception as e:
                    print(f"❌ [INGEST] Failed to check raw TikTok Ads ad insights table {raw_table_ad} existence due to {e}.")
                    logging.error(f"❌ [INGEST] Failed to check raw TikTok Ads ad insights table {raw_table_ad} existence due to {e}.")
                if not ingest_table_existed:
                    ...
                    if table_clusters_filtered:
                        table_configuration_defined.clustering_fields = table_clusters_filtered
                    try:
                        print(f"🔍 [INGEST] Creating raw TikTok Ads ad insights table {raw_table_ad} with partition on {table_partition_effective} and cluster on {table_clusters_filtered}...")
                        logging.info(f"🔍 [INGEST] Creating raw TikTok Ads ad insights table {raw_table_ad} with partition on {table_partition_effective} and cluster on {table_clusters_filtered}...")
                        table_metadata_defined = google_bigquery_client.create_table(table_configuration_defined)
                        print(f"✅ [INGEST] Successfully created raw TikTok Ads ad insights table {table_metadata_defined.full_table_id} with partition on {table_partition_effective} and cluster on {table_clusters_filtered}.")
                        logging.info(f"✅ [INGEST] Successfully created raw TikTok Ads ad insights table {table_metadata_defined.full_table_id} with partition on {table_partition_effective} and cluster on {table_clusters_filtered}.")
                    except Exception as e:
