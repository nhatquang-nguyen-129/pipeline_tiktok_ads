...
def staging_ad_insights() -> dict:
    ...
    # 1.2.9. Upload staging TikTok Ads ad insights to Google BigQuery            
        if not staging_table_exists:
            ...
            try:
                print(f"🔍 [STAGING] Overwriting {len(staging_df_enforced)} row(s) of staging TikTok Ads ad insights to existing Google BigQuery table {staging_table_ad}...")
                logging.warning(f"🔍 [STAGING] Overwriting {len(staging_df_enforced)} row(s) of staging TikTok Ads ad insights to existing Google BigQuery table {staging_table_ad}...")                    
                job_config = bigquery.LoadJobConfig(
                    write_disposition="WRITE_TRUNCATE",
                )
                load_job = google_bigquery_client.load_table_from_dataframe(
                ...
                raise RuntimeError(f"❌ [STAGING] Failed to overwrite {len(staging_df_enforced)} row(s) of staging TikTok Ads ad insights to existing Google BigQuery table {staging_table_ad} due to {e}.") from e       
